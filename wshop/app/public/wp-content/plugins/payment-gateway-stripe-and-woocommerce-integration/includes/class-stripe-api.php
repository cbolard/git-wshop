...
class EH_Stripe_Payment extends WC_Payment_Gateway {
    ...
    public function make_charge_params($charge_value, $order_id) {
        ...
        $charge_parsed = array(
            ...
            "source_type" => ('card' == $charge_data->payment_method_details->type ) ? ($charge_data->payment_method_details->card->brand . "( " . $charge_data->payment_method_details->card->funding . " )") : (( 'alipay' == $charge_data->payment_method_details->type ) ? 'Alipay' : (( 'sepa_debit' == $charge_data->payment_method_details->type ) ? 'Sepa Debit' : (('klarna' == $charge_data->payment_method_details->type ) ? 'Klarna' : (('afterpay_clearpay' == $charge_data->payment_method_details->type ) ? 'Afterpay' : (('wechat' == $charge_data->payment_method_details->type ) ? 'Wechat' : ('bacs_debit' == $charge_data->payment_method_details->type ? 'Bacs Debit' : ('sofort' == $charge_data->payment_method_details->type ? 'Sofort' : ('ideal' == $charge_data->payment_method_details->type ? 'iDEAL' : ('bancontact' == $charge_data->payment_method_details->type ? 'Bancontact' : ('eps' == $charge_data->payment_method_details->type ? 'EPS' : ('p24' == $charge_data->payment_method_details->type ? 'P24' : ('becs' == $charge_data->payment_method_details->type ? 'BECS' :  ('boleto' == $charge_data->payment_method_details->type ? 'Boleto' : ('oxxo' == $charge_data->payment_method_details->type ? 'OXXO' : ('grabpay' == $charge_data->payment_method_details->type ? 'Grabpay' : ('multibanco' == $charge_data->payment_method_details->type ? 'Multibanco' : 'Undefined') ))))))))))))))),
